...
3. Word count: 600 maximum; 1,600 for `04` and `09`.
...
- Status line missing → add line 3 exactly. Word count → cut, never split into a second file. Glossary → add the term to `09` in one sentence. Title near the name → remove the title, keep the name.
